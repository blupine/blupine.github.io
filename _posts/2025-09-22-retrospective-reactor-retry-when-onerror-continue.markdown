...
안녕하세요! 오늘은 Project Reactor를 사용하면서 겪었던 아주 **치명적인(Critical)** 장애 경험을 공유하려고 합니다.  
Reactor의 에러 처리 연산자인 `retryWhen`과 `onErrorContinue`를 함께 사용했다가, **스트림이 영원히 종료되지 않는(Hang)** 현상을 겪었던 이야기입니다.
...
로그를 확인해보니 에러는 분명 발생했습니다. 그런데 왜 스트림은 종료되지 않고 알림도 보내지 못했을까요?

---

## 2. 원인 분석: `retryWhen` + `onErrorContinue`

범인은 바로 `retryWhen`과 `onErrorContinue`의 잘못된 만남이었습니다.  
당시 코드는 대략 이런 형태였습니다.
...
Reactor에서 일반적인 연산자(`map`, `filter` 등)는 **위에서 아래(Upstream -> Downstream)** 로 데이터가 흐릅니다. 하지만 `onErrorContinue`는 아주 특수한 녀석입니다.

1. **`onErrorContinue`의 동작 방식**: 얘는 자기보다 **위(Upstream)** 에 있는 연산자들의 에러 처리 방식에 개입합니다. "위쪽에서 에러가 나도 스트림을 끊지 말고 다음 데이터를 달라"고 요청하죠.
2. **`retryWhen`의 내부 구조**: `retryWhen`은 재시도 로직을 수행하기 위해 내부적으로 `concatMap` 등을 사용해 복잡한 시그널 처리를 합니다.
3. **충돌 발생**:
    - `retryWhen`에서 재시도를 모두 실패(Exhausted)하고 에러를 터트리려고 합니다.
    - 그런데 밑에 있던 `onErrorContinue`가 "어? 에러 났네? 무시하고 계속 진행해(Swallow Error)"라며 에러를 삼켜버립니다.
    - 결과적으로 `retryWhen`은 에러를 밖으로 던지지도 못하고, 그렇다고 정상 완료(Complete) 시그널을 보내지도 못하는 상태가 되어버립니다.
    - **결국 시퀀스는 영원히 종료되지 않는 상태(Deadlock/Hang)가 됩니다.**

이 문제는 Reactor 커뮤니티에서도 꽤 유명한 이슈로, Reactor 메인테이너인 Simon Baslé는 Github 이슈에서 이런 명언(?)을 남기기도 했습니다.
...
결론부터 말씀드리면, **`retryWhen`과 `onErrorContinue`는 같이 쓰지 않는 것이 좋습니다.**  
대신 `onErrorResume`이나 `flatMap` 내부 처리를 통해 명시적으로 핸들링해야 합니다.

### 해결책 1: `onErrorContinue` 대신 `onErrorResume` 사용

가장 깔끔한 방법은 에러가 났을 때 "무시하고 계속(Continue)"하는 게 아니라, "대체값으로 복구(Resume)"하여 스트림을 정상 종료시키는 것입니다.
...
Reactor 에러 처리가 헷갈리신다면 아래 표를 기억해주세요.
...
이번 일을 계기로 "편리해 보이는 마법 같은 연산자(`onErrorContinue`)일수록 부작용을 조심해야 한다"는 교훈을 얻었습니다. 여러분의 운영 환경에서도 혹시 이 "위험한 동거"가 일어나고 있지는 않은지 한번 점검해보시길 바랍니다!
